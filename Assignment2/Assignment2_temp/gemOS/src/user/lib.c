...
{
   return _syscall4(SYSCALL_FTRACE, func_addr, action, nargs, fd_trace_buffer);
}


int create_trace_buffer(int mode)
{
  return _syscall1(SYSCALL_TRACE_BUFFER, mode);
}

int start_strace(int fd, int tracing_mode)
{
  return _syscall2(SYSCALL_START_STRACE, fd, tracing_mode);
}

int end_strace(void)
{
  return _syscall0(SYSCALL_END_STRACE);
}

int read_strace(int fd, void *buff, int count)
{
  return _syscall3(SYSCALL_READ_STRACE, fd, (u64)buff, count);
}

int read_ftrace(int fd, void *buff, int count)
{
  return _syscall3(SYSCALL_READ_FTRACE, fd, (u64)buff, count);
}


int strace(int syscall_num, int action)
{
  return _syscall2(SYSCALL_STRACE, syscall_num, action);
}
...
void set(char *ptr, char ch, int length)
 {
   int i;
   for (i = 0; i < length; ++i)
    	ptr[i] = ch;
...
int ustrncmp(char *s, char *d, int len){
	int ctr = 0;
	int ret = 0;
